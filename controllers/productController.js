...
export const fetchAllProducts = catchAsyncError(async (req, res, next) => {
  ...
  //Fetching top rating products(rating >= 4.5)
  const topRatingQuery = `
        SELECT p.*, COUNT(r.id) AS review_count
        FROM products p
        LEFT JOIN reviews r ON p.id = r.product_id
        WHERE p.ratings >= 4.5
        ...
        LIMIT 8
    `;
  const topRatingResult = await database.query(topRatingQuery);
  ...
  res.status(200).json({
    ...
    topRatingProducts: topRatingResult.rows,
  });
});
...
export const postProductReview = catchAsyncError(async (req, res, next) => {
  ...
  const purchasedCheckQuery = `
        ...
        WHERE o.buyer_id = $1 AND oi.product_id = $2 AND p.payment_status = 'Paid' LIMIT 1
    `;
...
export const fetchAIFilteredProduct = catchAsyncError(
  async (req, res, next) => {
    ...
    const filteredProduct = result.rows;
